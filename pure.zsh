...
prompt_pure_state_setup() {
  ...
  # lang version
  RUBY_VERSION_STRING="rb:"$(~/.rbenv/bin/rbenv version-name)
  NODE_VERSION_STRING="nd:"$(~/.nodenv/bin/nodenv version-name)
  PYTHON_VERSION_STRING="py:"$(~/.pyenv/bin/pyenv version-name)

	typeset -gA prompt_pure_state
	prompt_pure_state[version]="1.10.3"
	prompt_pure_state+=(
    username "$username%F{yellow}(${RUBY_VERSION_STRING} ${PYTHON_VERSION_STRING} ${NODE_VERSION_STRING})"
